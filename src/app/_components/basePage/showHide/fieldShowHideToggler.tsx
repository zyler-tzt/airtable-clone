...
export function FieldShowHideToggler({viewFields, viewId, fieldId, fieldName, tableColumns, setTableColumns, fieldType, tableId}: FieldShowHideTogglerProps) {
    ...
    function handleFieldToggle(fieldId: number, isChecked: boolean) {
        if (isChecked) {
            const copy = [...tableColumns, {id: fieldId, type: fieldType, name: fieldName, tableId: tableId}]
    ...
    return (
        <div className="px-2 py-1 flex flex-row gap-2">
            <Switch key={`switch-${fieldId}`} className="h-4 w-7 data-[state=checked]:bg-green-700" defaultChecked={!!exist} onCheckedChange={(checked) => handleFieldToggle(fieldId, checked)}/>
            <Label htmlFor="airplane-mode" className="text-xs">{fieldName}</Label>
        </div>
    )
}